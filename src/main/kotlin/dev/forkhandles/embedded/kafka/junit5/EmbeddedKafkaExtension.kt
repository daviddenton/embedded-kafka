...
import dev.forkhandles.embedded.support.findAnnotatedMethods
import dev.forkhandles.embedded.support.findAnnotation
import dev.forkhandles.embedded.support.getAnnotation
...
import java.util.*
import dev.forkhandles.embedded.kafka.EmbeddedKafka as EmbeddedKafkaService
...
val ExtensionContext.clientProperties: Map<String, Any>
    get() = mapOf(BOOTSTRAP_SERVERS_CONFIG to this.embeddedKafka.bootstrapServers)
...
class EmbeddedKafkaExtension : TestInstancePostProcessor, BeforeEachCallback, AfterEachCallback, ParameterResolver {
    ...
    private fun postProcessTopics(context: ExtensionContext, testInstance: Any) {
        context.getStore(NAMESPACE).let { store ->
            ...
            if (newTopics.isNotEmpty()) {
                store.topics += newTopics.associateBy { it.name() }
            }
        }
    }
    ...
    override fun beforeEach(context: ExtensionContext) {
        context.getStore(NAMESPACE).let { store ->
            ...
            val embeddedKafka = EmbeddedKafkaService(
                    zkConnectSupplier = { embeddedZookeeper.zkConnect },
    ...
    override fun resolveParameter(parameterContext: ParameterContext, extensionContext: ExtensionContext): Any? =
            argumentResolvers
                    ...
                    .also { (it as? AutoCloseable)?.let { extensionContext.registerCloseable(it) } }
                    .let { (it as? DisposableParameter<*>)?.value ?: it }
    /**
